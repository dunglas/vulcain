<h1 align="center"><img src="public/vulcain.svg" alt="vulcain: Live Updates Made Easy" title="Live Updates Made Easy"></h1>

*Protocol and Reference Implementation*

vulcain is a protocol allowing to push data updates to web browsers and other HTTP clients in a convenient, fast, reliable and battery-efficient way.
...
A reference, production-grade, implementation of **a vulcain hub** (the server) is also available here.
It's a free software (AGPL) written in Go. It is provided along with a library that can be used in any Go application to implement the vulcain protocol directly (without a hub) and an official Docker image.

[Try the demo!](https://demo.vulcain.rocks/)

In addition, a managed and high-scalability version of vulcain is [available in private beta](mailto:dunglas+vulcain@gmail.com?subject=I%27m%20interested%20in%20vulcain%27s%20private%20beta).

## vulcain in a Few Words
...
Optionally, [the authorization mechanism](https://github.com/dunglas/vulcain/blob/master/spec/vulcain.md#authorization) can be used to subscribe to private updates.
...
```javascript
fetch('https://example.com/books/1') // Has this header `Link: <https://example.com/hub>; rel="vulcain"`
    .then(response => {
        // Extract the hub URL from the Link header
        const hubUrl = response.headers.get('Link').match(/<([^>]+)>;\s+rel=(?:vulcain|"[^"]*vulcain[^"]*")/)[1];
...
const req = https.request({
    ...
    headers: {
        ...
        // the JWT must have a vulcain.publish key containing an array of targets (can be empty for public updates)
...
* [Python/Flask: chat application](examples/chat-python-flask) ([live demo](https://python-chat.vulcain.rocks/))
...
Example of usage: the vulcain integration in [API Platform](https://api-platform.com/docs/client-generator):
...
**vulcain gets you covered!**

## Protocol Specification

The full protocol specification can be found in [`spec/vulcain.md`](spec/vulcain.md).
It is also available as an [IETF's Internet Draft](https://datatracker.ietf.org/doc/draft-dunglas-vulcain/),
...
A managed, high-scalability version of vulcain is available in private beta.
[Drop us a mail](mailto:dunglas+vulcain@gmail.com?subject=I%27m%20interested%20in%20vulcain%27s%20private%20beta) for details and pricing.

### Usage

#### Prebuilt Binary

Grab the binary corresponding to your operating system and architecture [from the release page](https://github.com/dunglas/vulcain/releases), then run:

    JWT_KEY='myJWTKey' ADDR=':3000' DEMO=1 ALLOW_ANONYMOUS=1 CORS_ALLOWED_ORIGINS=* PUBLISH_ALLOWED_ORIGINS='http://localhost:3000' ./vulcain
...
To run it in production mode, and generate automatically a Let's Encrypt TLS certificate, just run the following command as root:
    ...
    JWT_KEY='myJWTKey' ACME_HOSTS='example.com' ./vulcain
...
See [the protocol](spec/vulcain.md) for further informations.
...
To install vulcain in a [Kubernetes](https://kubernetes.io) cluster, use the official [Helm Chart](https://hub.helm.sh/charts/stable/vulcain):

    helm install stable/vulcain
...
If you're having trouble getting the Hub running, you may have set an incorrect value for the environment variable `ADDR`. Use `ADDR=":3000"` (and not `ADDR="localhost:3000"`). Windows may ask you for allowing `vulcain.exe` in your firewall.
...
If subscribing to the `EventSource` in the browser doesn't work (the browser instantly disconnects from the stream or complains about CORS policy on receiving an event), check that you've set a proper value for `CORS_ALLOWED_ORIGINS` on running vulcain. It's fine to use `CORS_ALLOWED_ORIGINS=*` for your local development.

#### URI Template and Topics

Try [our URI template tester](https://uri-template-tester.vulcain.rocks/) to ensure that the template matches the topic.

## FAQ

### How to Use vulcain with GraphQL?

Because they are delivery agnostic, vulcain plays particulary well with [GraphQL's subscriptions](https://facebook.github.io/graphql/draft/#sec-Subscription).

In response to the subscription query, the GraphQL server may return a corresponding topic URL.
The client can then subscribe to the vulcain's event stream corresponding to this subscription by creating a new `EventSource` with an URL like `https://example.com/hub?topic=https://example.com/subscriptions/<subscription-id>` as parameter.

Updates for the given subscription can then be sent from the GraphQL server to the clients through the vulcain hub (in the `data` property of the server-sent event).

To unsubscribe, the client just calls `EventSource.close()`.

vulcain can easily be integrated with Apollo GraphQL by creating [a dedicated transport](https://github.com/apollographql/graphql-subscriptions).

### How to Monitor the Hub Using Supervisor

Use the following file as a template to run the vulcain hub with [Supervisor](http://supervisord.org):

```ini
[program:vulcain]
command=/path/to/vulcain
...
stdout_logfile=/path/to/logs/vulcain.out.log
stderr_logfile=/path/to/logs/vulcain.error.log
```

Save this file to `/etc/supervisor/conf.d/vulcain.conf`.
Run `supervisorctl reread` and `supervisorctl update` to activate and start the vulcain hub.
...
server {
    ...
    location / {
        proxy_pass http://url-of-your-vulcain-hub;
...
### What's the Difference Between vulcain and WebSocket?

[WebSocket](https://developer.mozilla.org/en-US/docs/Web/API/WebSockets_API) is a low level protocol, vulcain is a high level one.
vulcain provides convenient built-in features such as authorization, re-connection and state reconciliation ; while with WebSocket, you need to implement them yourself.
Also, unlike vulcain (which is built on top of HTTP and Server-Sent Events), WebSocket [is not designed to leverage HTTP/2](https://www.infoq.com/articles/websocket-and-http2-coexist).

HTTP/2 connections are multiplexed and bidirectional by default (it was not the case of HTTP/1).
When using vulcain over a h2 connection (recommended), your app can receive data through Server-Sent Events, and send data to the server with regular `POST` (or `PUT`/`PATCH`/`DELETE`) requests, with no overhead.

Basically, in most cases vulcain can be used as a modern and easier to use replacement for WebSocket.

### What's the Difference Between vulcain and WebSub?

[WebSub](https://www.w3.org/TR/websub/) is a server-to-server only protocol, while vulcain is also a server-to-client and client-to-client protocol.

vulcain has been heavily inspired by WebSub, and we tried to make the protocol as close as possible from the WebSub one.

vulcain uses Server-Sent Events to dispatch the updates, while WebSub use `POST` requests. Also, vulcain has an advanced authorization mechanism, and allows to subscribe to several topics with only one connection using URI templates.

### What's the Difference Between vulcain and Web Push?
...
On the other hand, vulcain is a duplex protocol designed to send live updates to devices currently connected to the web or mobile app. The payload is not limited, and the message goes directly from your servers to the clients.

In summary, use the Push API to send notifications to offline users (that will be available in Chrome, Android and iOS's notification centers), and use vulcain to receive and publish live updates when the user is using the app.

## Tools

* [PHP library to publish vulcain updates](https://github.com/symfony/vulcain)
* [Official vulcain support for the Symfony framework](https://github.com/symfony/vulcain-bundle)
* [Official vulcain support for the API Platform framework](https://api-platform.com/docs/core/vulcain/)
* [Laravel vulcain Broadcaster](https://github.com/mvanduijker/laravel-vulcain-broadcaster)
...
* [Python library to publish and consume vulcain updates](https://github.com/vitorluis/python-vulcain)

## Learning Resources

### English

* [Official Push and Real-Time Capabilities for Symfony and API Platform using vulcain (Symfony blog)](https://dunglas.fr/2019/03/official-push-and-real-time-capabilities-for-symfony-and-api-platform-vulcain-protocol/)
* [Tech Workshop: vulcain by Kevin Dunglas at SensioLabs (SensioLabs)](https://blog.sensiolabs.com/2019/01/24/tech-workshop-vulcain-kevin-dunglas-sensiolabs/)
* [Real-time messages with vulcain using Laravel](http://thedevopsguide.com/real-time-notifications-with-vulcain/)

### French

* [Tutoriel vidéo : Notifications instantanées avec vulcain (Grafikart)](https://www.grafikart.fr/tutoriels/symfony-vulcain-1151)
* [vulcain, un protocole pour pousser des mises à jour vers des navigateurs et app mobiles en temps réel (Les-Tilleuls.coop)](https://les-tilleuls.coop/fr/blog/article/vulcain-un-protocole-pour-pousser-des-mises-a-jour-vers-des-navigateurs-et-app-mobiles-en-temps-reel)
